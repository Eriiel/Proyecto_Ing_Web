...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['statuses'])) {
    ...
    foreach ($_POST['statuses'] as $solicitud_id => $nuevo_estado) {
        ...
        $stmt = $conn->prepare("UPDATE solicitudes SET estado_solicitud = ? WHERE id = ?");
        if ($stmt) {
    ...
    $_SESSION['flash_message'] = [
        'type' => 'success',
        'text' => '¡Los estados de las solicitudes han sido actualizados!'
    ];
...
if (!$result) {
    die("Error al consultar solicitudes: " . $conn->error);
}
